...
const roles = ['Creative Industrialist', 'Brand Strategist', 'Storyteller', 'Experience Curator', 'Host', 'CEO', 'Founder'];
...
const clients = [
  'Policy Innovation Center', 'Rayven Strategic Communications', 'NXT Wave Africa', 'George Okoro Studios', 'TCN Abuja', 'Equity Circle', 'Beths Protein', 'Daurama Foundation', 'Integrity Foundation', 'Rhema Chapel Churches', 'Sugar Mining', 'Servelead Global', 'Servelead Humanitarian Initiative', 'Laicos Farms', 'Timewise Logistics', 'Respectech HR', 'Bukkateria', 'Chefs Academy', 'Abuja Food and Fashion Festival', 'Zinger Wallet', 'Ese Tonia Foundation', 'Bon Hotel Asokoro', 'Edo State Government', 'MWFAAN', 'Perspective with the Rayven', 'Renmoney', 'Elite Life Consulting',
];
...
export default function Home() {
  ...
  const companyVentures = getCompaniesByType('company').map((c, i) => ({
    num: String(i + 1).padStart(2, '0'), id: c.id, name: c.name, role: c.shortName === 'PWM' ? 'CEO' : 'Founder', desc: c.overview,
  }));
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.fromTo(heroTitleRef.current.children, { y: 100, opacity: 0, rotateX: -15 }, { y: 0, opacity: 1, rotateX: 0, duration: 1.2, stagger: 0.15, ease: 'power4.out' })
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      stats.forEach((stat, i) => {
        ...
        if (!el) return;
        const obj = { val: 0 };
        gsap.to(obj, { val: stat.num, duration: 2, ease: 'power2.out', scrollTrigger: { trigger: el, start: 'top 85%', once: true }, onUpdate: () => { el.textContent = obj.val >= stat.num * 0.99 ? stat.display : Math.floor(obj.val).toLocaleString() + stat.suffix; } });
      });
    }, counterRef);
    return () => ctx.revert();
  }, []);
    ...
    <div>
      ...
      <section className="relative min-h-screen flex items-end overflow-hidden" aria-label="Hero">
        ...
        <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-10 pb-20 md:pb-32 pt-36 w-full">
          ...
          <div ref={heroCtaRef} className="flex flex-wrap gap-4">
            <Link to="/services" className="inline-flex items-center gap-3 bg-[#D4AF37] text-white px-8 py-4 text-[11px] font-bold tracking-ultrawide uppercase hover:bg-[#e0bd4a] transition-all duration-300 hover:translate-y-[-2px] hover:shadow-[0_10px_40px_rgba(14,86,0,0.3)]" style={{ opacity: 0 }}>
              Explore Services <ArrowRight className="w-4 h-4" />
            </Link>
            <Link to="/contact" className="inline-flex items-center gap-3 border border-white/20 text-white px-8 py-4 text-[11px] font-bold tracking-ultrawide uppercase hover:border-[#D4AF37] hover:text-[#D4AF37] transition-all duration-300 hover:translate-y-[-2px]" style={{ opacity: 0 }}>
              Get in Touch
            </Link>
          </div>
          <div ref={heroTagsRef} className="flex flex-wrap gap-2 mt-14">
            {roles.map((tag) => (
              <span key={tag} className="hero-tag px-4 py-1.5 border border-white/10 text-white/40 text-[10px] tracking-ultrawide uppercase hover:border-[#D4AF37]/50 hover:text-[#D4AF37] transition-all duration-300 cursor-default" style={{ opacity: 0 }}>{tag}</span>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-20 md:py-32" aria-label="Services overview">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          ...
          <p className="text-black/60 text-center max-w-xl mx-auto mb-10 text-sm leading-relaxed">Needing clarity and support? Here is how Pamela transforms your brand, business, and presence.</p>
          <div ref={servicesGridRef} className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
            {services.map((s) => (
              <Link key={s.num} to="/services" className="service-card group relative p-7 border border-black/5 hover:border-[#D4AF37]/50 transition-all duration-700 cursor-pointer overflow-hidden block" style={{ opacity: 0 }}>
          ...
          <div className="text-center mt-10">
            <Link to="/services" className="inline-flex items-center gap-3 text-black text-[11px] font-bold tracking-ultrawide uppercase border-b border-black/20 pb-1 hover:border-[#D4AF37] hover:text-[#D4AF37] transition-all duration-300 group">
              View Full Service Details & Pricing <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform duration-300" />
            </Link>
          </div>
        </div>
      </section>
        ...
        {/* ═══════ LOGO SCROLL ═══════ */}
      <section className="bg-black py-2 border-y border-white/10" aria-label="Company logos">
        <div className="overflow-hidden relative">
          {/* Smooth fading edges so logos don't pop in/out abruptly */}
          <div className="absolute left-0 top-0 bottom-0 w-40 md:w-64 bg-gradient-to-r from-black to-transparent z-10 pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-40 md:w-64 bg-gradient-to-l from-black to-transparent z-10 pointer-events-none" />
          
          <div className="flex animate-marquee whitespace-nowrap">
            {[logo1, logo2, logo3, logo4, logo1, logo2, logo3, logo4, logo1, logo2, logo3, logo4].map((logo, i) => (
              <div key={i} className="flex items-center justify-center mx-14 md:mx-24 lg:mx-16 shrink-0">
                <img 
                  src={logo} 
                  alt="Company Logo" 
                  className="h-28 md:h-36 lg:h-48 w-auto object-contain transition-all duration-500 hover:scale-105 hover:opacity-100"
                  style={{ 
      ...
      <section className="relative min-h-[85vh] md:min-h-[90vh] overflow-hidden" aria-label="About Pamela Williams">
        <div className="absolute inset-0">
          <img src={img6} alt="" className="w-full h-full object-cover object-right" loading="eager" aria-hidden="true" />
        ...
        <div className="relative z-10 min-h-[85vh] md:min-h-[90vh] flex flex-col justify-end max-w-[1400px] mx-auto px-6 lg:px-10 pb-16 md:pb-24 pt-40 md:pt-56">
          <div className="max-w-2xl">
            ...
            <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white leading-[0.95] tracking-tight mb-8">
              I don&apos;t just build brands — <span className="text-[#D4AF37] italic">I build</span> <span className="text-[#D4AF37] italic">ecosystems.</span>
            </h2>
            ...
            <Link to="/about" className="inline-flex items-center gap-3 text-[#D4AF37] text-[11px] font-bold tracking-ultrawide uppercase border-b border-[#D4AF37]/30 pb-1 hover:border-[#D4AF37] transition-all duration-300 group">
              Learn More About Me <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform duration-300" />
            </Link>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map((stat, i) => (
              <div key={stat.label} className="text-center min-w-0">
                <span ref={(el) => (countersRef.current[i] = el)} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white block mb-2">0{stat.suffix}</span>
                <p className="text-white/50 text-[10px] tracking-ultrawide uppercase font-semibold">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* VENTURES */}
      <section className="bg-white py-16 md:py-28" aria-label="Business ventures">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <p className="text-[#D4AF37] text-[10px] tracking-ultrawide uppercase mb-4 font-bold text-center">Ventures</p>
          <h2 className="text-2xl md:text-4xl font-black text-black text-center mb-3">Brands We&apos;ve Built</h2>
          <p className="text-black/50 text-center max-w-lg mx-auto mb-12 text-sm leading-relaxed">A multi-sector ecosystem of brands reflecting passion for creativity, strategy, lifestyle, and community.</p>
          <div ref={venturesRef} className="space-y-3">
            {companyVentures.map((v) => (
              <Link key={v.id} to={`/companies/${v.id}`} className="venture-card group grid md:grid-cols-12 gap-3 md:gap-6 items-center p-5 md:p-7 border border-black/5 hover:border-[#D4AF37]/40 transition-all duration-600 block" style={{ opacity: 0 }}>
                <div className="md:col-span-1"><span className="text-[#D4AF37]/70 text-xl md:text-2xl font-black group-hover:text-[#D4AF37] transition-colors duration-600">{v.num}</span></div>
                <div className="md:col-span-3">
                  <h3 className="text-lg font-bold text-black/90 group-hover:text-[#D4AF37] transition-colors duration-600">{v.name}</h3>
                  <p className="text-[#D4AF37]/70 text-[10px] font-bold tracking-ultrawide uppercase mt-0.5">{v.role}</p>
                </div>
                <div className="md:col-span-7"><p className="text-black/35 group-hover:text-black/55 text-sm leading-relaxed transition-colors duration-600">{v.desc}</p></div>
                <div className="md:col-span-1 flex justify-end"><ArrowRight className="w-4 h-4 text-[#D4AF37]/50 group-hover:text-[#D4AF37] group-hover:translate-x-0.5 transition-all duration-300 shrink-0" /></div>
              </Link>
            ))}
          </div>
          <div className="text-center mt-10">
            <Link to="/companies" className="inline-flex items-center gap-3 text-black text-[11px] font-bold tracking-ultrawide uppercase border-b border-black/20 pb-1 hover:border-[#D4AF37] hover:text-[#D4AF37] transition-all duration-300 group">
              View All Companies & Communities <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform duration-300" />
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          <div className="grid lg:grid-cols-5 gap-12 lg:gap-16">
            <div className="lg:col-span-2">
              <div className="sticky top-28">
                ...
                <div className="mt-4 flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#D4AF37] flex items-center justify-center"><span className="text-white text-xs font-black">150+</span></div>
            ...
            <div className="lg:col-span-3">
              ...
              <p className="text-black/60 mb-10 leading-relaxed">From diplomatic events with the French Embassy to hosting 1,500+ attendee tech conferences, managing the Mandela Washington Fellowship 10th anniversary, and building partnerships with institutions like the National Gallery of Art — Pamela brings precision, creativity, and authority to every stage and project.</p>
              <div ref={eventsListRef}>
                {events.map((event, i) => (
                  <div key={i} className="event-item flex items-start gap-5 py-4 border-b border-black/5 group cursor-default">
                    <span className="text-[#D4AF37] font-mono text-sm mt-0.5 shrink-0 group-hover:text-[#e0bd4a] transition-colors duration-300">{String(i + 1).padStart(2, '0')}</span>
      ...
      <section className="bg-black py-20 md:py-32" aria-label="Client testimonials">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          ...
          <p className="text-white/50 text-center max-w-xl mx-auto mb-10 text-sm leading-relaxed">Don&apos;t just take our word for it — hear from the brands, executives, and organizations that have experienced the Pamela Williams difference.</p>
          <div ref={testimonialsGridRef} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {testimonials.map((t) => (
              <blockquote key={t.name} className="testimonial-card group p-7 border border-white/10 hover:border-[#D4AF37]/40 transition-all duration-700 flex flex-col" style={{ opacity: 0 }}>
                <div className="flex gap-0.5 mb-4">{[...Array(5)].map((_, j) => (<svg key={j} className="w-3 h-3 text-[#D4AF37]" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>))}</div>
                <p className="text-white/40 group-hover:text-white/60 text-sm leading-relaxed flex-1 mb-5 italic transition-colors duration-500">&ldquo;{t.text}&rdquo;</p>
                <footer className="border-t border-white/10 pt-4"><cite className="not-italic"><span className="font-bold text-white/80 text-sm group-hover:text-[#D4AF37] transition-colors duration-500">{t.name}</span></cite></footer>
              </blockquote>
            ))}
          </div>
        </div>
      ...
      <section className="bg-white py-16 md:p-24" aria-label="Client brands">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10">
          ...
          <p className="text-black/60 text-center max-w-lg mx-auto mb-4 text-sm leading-relaxed">Over 200 brands and organizations have trusted Pamela to elevate their visibility, sharpen their identity, and scale their influence.</p>
          ...
          <div ref={clientsRef} className="flex flex-wrap justify-center gap-2.5">
            {clients.map((c) => (
              <span key={c} className="client-chip px-4 py-2 border border-black/10 text-black/40 text-xs hover:border-[#D4AF37] hover:text-[#D4AF37] transition-all duration-500 cursor-default" style={{ opacity: 0 }}>{c}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="relative py-24 md:py-32 bg-black overflow-hidden" aria-label="Booking call to action">
        <div className="absolute inset-0 opacity-15"><img src={img2} alt="" className="w-full h-full object-cover" loading="lazy" aria-hidden="true" /></div>
        <div className="relative z-10 max-w-3xl mx-auto px-6 lg:px-10 text-center">
          ...
          <h2 className="text-3xl sm:text-5xl md:text-6xl font-black text-white leading-tight mb-6">Ready to<br /><span className="text-[#D4AF37]">Level Up?</span></h2>
          <p className="text-white/40 text-lg leading-relaxed mb-10">Pamela didn&apos;t just step into the world of media, branding, and business — she built her own table, set the stage, and invited others to thrive alongside her. Now it&apos;s your turn.</p>
          <button onClick={openModal} className="inline-flex items-center gap-3 bg-[#D4AF37] text-white px-10 py-4 text-[11px] font-bold tracking-ultrawide uppercase hover:bg-[#e0bd4a] transition-all duration-300 hover:translate-y-[-2px] hover:shadow-[0_10px_40px_rgba(14,86,0,0.3)] cursor-pointer">
